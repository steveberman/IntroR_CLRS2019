...
Here, `e` is a vector with `N` random normally distributed values. `X1` is the sequence of integers from 1 through 10. 
...
* Possible to add metadata to variables (like names) via attributes
...
election_years <- seq(from=2000, to=2020, by=4)
i <- 1:5
year = 2000:2004
```
...
The `c()` function will concatenate scalars or vectors to form a vector.
...
The `which` function returns indices that match a logical expression (rather than the values).
...
Vectors may be used in arithmetic operations.  A large number of functions in R are vectorized, meaning that they can act on vectors and return vectors. 
...
B0 and B1 are scalars. X1 and e are vectors of length 100. 
...
* `union`
* `intersect`
* `setdiff` - find differences
* `setequal` - test for equality
* `is.element` - search for single element
...
Lists have data of arbitrary complexity. Any type, any length. In a sense, they are vectors of variants.  Note the new `[[ ]]` double bracket operator.
